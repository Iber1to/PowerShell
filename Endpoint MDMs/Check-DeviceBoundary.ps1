<#
.AUTHOR
       Powered by Iber0
       Website: https://www.linkedin.com/in/alejandro-aguado-08882a31/

.SYNOPSIS
        Check if the device is in any boundary

.DESCRIPTION
      The script tests all hosts in a given collection. It checks if the machine has reported an IP to SCCM and if so if it falls into a boundary
      or belongs to an unknown subnet that should be registered.
      The script leaves 2 files. One with the devices that do not fall in any boundary. Another one with the hosts that either have no IP or have an IP that falls in a boundary.
      The script is initially intended for a collection of devices that do not show boundary group so if it falls in a boundary but does not show boundary group
      either the boundary is misconfigured and not associated with a boundary group or the client is broken.
      Leave the file with the results in C:\temp. If the directory does not exist, create it.

.VERSION HISTORY
        V1.0   23 March 2021 -  Temporarily in this version the site is requested. This must be removed when PL1 has all its boundaries in the IP range type.
        V1.1   31 July 2021  -  Check the Boundarys of all sites. The Main Collection is left as default.    
#>

# Declare functions
# Function to log
function Write-CMTracelog{

    <#
    .SYNOPSIS
         Add an entry to a log file
    
    .DESCRIPTION
        Add entries to an existing log file or create a new one.
    
    .PARAMETER Path
        File path. If omitted, the PathCMTracelog parameter must be set.
 
    .PARAMETER PathCMTracelog
        Variable to declare in the script that uses the function with the path to the log file.
        This is used to avoid constantly passing the 'Path' parameter. 
    
    .PARAMETER Message
        Content to be added to the log entry.
    
    .PARAMETER Component
        System component that creates the log entry. By default it adds the function itself.
    
    .PARAMETER Type
        Type of message to add. The default value is 'Information' . The other values are Warning' and 'Error'.
    
    .EXAMPLE
         $PathCMTracelog= 'C:\Temp\logalternativo.log'
         Write-CMTracelog 'This is a default test message'
         Output= "This is a default test message  Write-CMTracelog 31/08/2021 13:21:20 0 (0x0)"
    
    .EXAMPLE
        Write-CMTracelog -Path C:\Temp\logalternativo.log -Message $error[0] -Type Error -Component $MyInvocation.MyCommand.Name
    
    
    .NOTES
        Author:  Alejandro Aguado Garcia
        Website: https://www.linkedin.com/in/alejandro-aguado-08882a31/
        Twitter: @Alejand94399487
    #> 
    
    
    
        [CmdletBinding()]
        Param(
    
              [Parameter(Mandatory=$true)]
              [String]$Message,
                
              [Parameter()]
              [ValidateNotNullOrEmpty()]
              [String]$Path,
                      
              [Parameter()]
              [ValidateNotNullOrEmpty()]
              [String]$Component= $MyInvocation.MyCommand.Name,
    
              [Parameter()]
              [ValidateNotNullOrEmpty()]
              [ValidateSet("Information", "Warning", "Error")]
              [String]$Type = 'Information'
        )
    
        if(!$Path){
            $Path= $PathCMTracelog
            }
            
        switch ($Type) {
            "Info" { [int]$Type = 1 }
            "Warning" { [int]$Type = 2 }
            "Error" { [int]$Type = 3 }
        }
    
        # Create register in CMTrace format
        $Content = "<![LOG[$Message]LOG]!>" +`
            "<time=`"$(Get-Date -Format "HH:mm:ss.ffffff")`" " +`
            "date=`"$(Get-Date -Format "M-d-yyyy")`" " +`
            "component=`"$Component`" " +`
            "context=`"$([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)`" " +`
            "type=`"$Type`" " +`
            "thread=`"$([Threading.Thread]::CurrentThread.ManagedThreadId)`" " +`
            "file=`"`">"
    
        # Add line to log 
        Add-Content -Path $Path -Value $Content
    }
 
 # Function to connect SCCM
    function Connect-CMSite{ 
       <#
       .SYNOPSIS
           Connect to SCCM site
       
       .DESCRIPTION
           Connect to SCCM site and load Configmanager module
       
       .PARAMETER SiteCode
           Site code to connect
       
       .PARAMETER ProviderMachineName
            FQDN site server 
       
       .EXAMPLE
            Connect-CMSite
       
       .EXAMPLE'
            Connect-CMSite -SiteCode 'CM1' -ProviderMachineName 'yourserver.contoso.local'
       
       .NOTES
           Author:  Alejandro Aguado Garcia
           Website: https://www.linkedin.com/in/alejandro-aguado-08882a31/
           Twitter: @Alejand94399487
       #> 
       
       [CmdletBinding()] 
           param (
               [ValidateNotNullOrEmpty()]
               [string]$SiteCode,
               [string]$ProviderMachineName                 
                 )
       
       # Import ConfigurationManager.psd1 module 
       if($null -eq (Get-Module ConfigurationManager)) {
           Import-Module "$($ENV:SMS_ADMIN_UI_PATH)\..\ConfigurationManager.psd1"
       }
       
       # Assemble the site unit if it does not exist
       if($null -eq (Get-PSDrive -Name $SiteCode -PSProvider CMSite -ErrorAction SilentlyContinue)) {
           New-PSDrive -Name $SiteCode -PSProvider CMSite -Root $ProviderMachineName
       }
       
       # Set site code
       Set-Location "$($SiteCode):\" 
       }
 
 # The function is responsible for checking if an IP is within a boundary range. It needs 3 parameters. IP to check, initial IP of the range, final IP of the range.
 Function IsIpAddressInRange {
    param(
            [string] $ipAddress,
            [string] $fromAddress,
            [string] $toAddress
        )
     
        $ip = [system.net.ipaddress]::Parse($ipAddress).GetAddressBytes()
        [array]::Reverse($ip)
        $ip = [system.BitConverter]::ToUInt32($ip, 0)
     
        $from = [system.net.ipaddress]::Parse($fromAddress).GetAddressBytes()
        [array]::Reverse($from)
        $from = [system.BitConverter]::ToUInt32($from, 0)
     
        $to = [system.net.ipaddress]::Parse($toAddress).GetAddressBytes()
        [array]::Reverse($to)
        $to = [system.BitConverter]::ToUInt32($to, 0)
     
        $from -le $ip -and $ip -le $to
    }
 
 #Mail Function with template corporative
 function send-MailHtml{
    <#
    .SYNOPSIS
        Send an email with the corporate template.
    
    .DESCRIPTION
        Send an email with the corporate template format.
         The HTML titles H2 and H4 are used.  
    
    .PARAMETER Subject
        Subject that will appear in the email. Mandatory parameter. 
    
    .PARAMETER ListAdress
        List of addresses to send to. Supports multiple addresses. Mandatory parameter.
    
    .PARAMETER Title
        Text to appear in the body of the message with title format <H2>. Optional.
    
    .PARAMETER Message
        Text to appear in the body of the message below the title in <p> format. Optional.
    
    .PARAMETER attachment
        Attachment file to email. 
       
    .EXAMPLE
        send-MailHtml -Subject "Tarea programada ejecutada" -ListAdress "alejandro.aguado@contoso.com"
        send-MailHtml -Subject "Tarea programada ejecutada" -ListAdress "alejandro.aguado@contoso.com" -Title "Informacion sobre tareas programadas" -Message "Esta es una tarea automatizada" -attachment "C:\temp\text.txt"
    
    
    .NOTES
        Author:  Alejandro Aguado Garcia
        Website: https://www.linkedin.com/in/alejandro-aguado-08882a31/
        Twitter: @Alejand94399487
    #> 
    
        [CmdletBinding()]
        Param(
    
              [Parameter(Mandatory = $true)]
              [String]$Subject, 
              [string[]]$ListAdress,
              [string]$SmtpServer,
    
              [Parameter()]
              [ValidateNotNullOrEmpty()]
              [String]$Title,
              [String]$Message,
              [string[]]$attachment
        )
    
        #Mail variables.
        $scriptFrom = $myInvocation.ScriptName
        $LaunchDate = Get-Date
        $MailSender = "SCCM Monitoring <automatic_sender@econtoso.com>"
    
        #Body message
        $htmlBody = '<p>&nbsp;</p>
        <h2>{0}</h2>
        <p>{1}</p>
        <p>The Script: {3} are work fine.<br>
        Execution date: {2}<br>
        Please this is an automated message. Please do not reply to this email.<br></p>
        <table style="height: 25px;" width="100%">
        <tbody>
        <tr>
        <td style="text-align: left;" colspan="3"><img id="Umbrella Corp" src="data:image/jpeg;base64," alt="Umbrella Corp"/>
        </td>
        </tr>
        </tbody>
        </table>'
    
    
     
        if ($attachment) {
            Send-MailMessage  -From $MailSender -to $ListAdress -SmtpServer $SmtpServer -Subject $Subject -Attachments $attachment -BodyAsHtml ($htmlBody -f $Title, $Message, $LaunchDate, $scriptFrom)
        }
        else {
            Send-MailMessage  -From $MailSender -to $ListAdress -SmtpServer $SmtpServer -Subject $Subject -BodyAsHtml ($htmlBody -f $Title, $Message, $LaunchDate, $scriptFrom)
        }
}
 
 # Configuration mandatory
 $PathCMTracelog = 'C:\Temp\TestBoundarys.log'
 $TargetCollection = 'CAS000001' # Change the value of $TargetCollection to the ID of the collection with the devices you want to check..
 $SccmServer = "primaryserver.contoso.com" # Server SCCM to connect
 $SiteCode = "CM1" # SCCM Site Code 
 $PathFilesExport = "C:\temp\results\" # Folder to export results
 [string[]]$ListAdress= "Alejandro Aguado Garcia <alejandro.aguado@contoso.com>" # List of e-mail recipients
 $SmtpServer = "mail.contoso.com"
 
 
 if( $(Test-Path -Path $PathCMTracelog)){Remove-Item -Path $PathCMTracelog -Force}
 Write-CMTracelog 'Start Script'
 Write-CMTracelog "Connecting to $SiteCode"
 Connect-CMSite -SiteCode $SiteCode -ProviderMachineName $SccmServer
 
 Write-CMTracelog 'Load device list'
 $ListadoFinal= @()
 $ListResourceID= @()
 $Listboundaries= @()
 $ListadoClientesRepairs= @()
 
 # Load device in $TargetCollection
 $ListResourceID= Get-CMDevice -CollectionId $TargetCollection -fast -Resource |Select-Object name, IPAddresses
 
 # Load boundary list
 Write-CMTracelog 'Load boundary list'
 $Listboundaries= Get-CMBoundary | Where-Object {$_.BoundaryType -eq 3}
 
 # Check if device have boundary
 Write-CMTracelog 'Check if device have boundary. This procees can take hours. '
 foreach($ResourceID in $ListResourceID)
     {
      # Checks if the IP is in range, returns 'True' if it is and 'False' if it is not.
      $FalseCount=0 # Counter for boundaries where there has been no correspondence
      # I only process devices with IP's. If the device has a blank IP, the check function fails by passing a $null parameter.
      If($ResourceID.IPAddresses[0])
         {
          Write-CMTracelog "Cheking $($ResourceID.name) with IP: $($ResourceID.IPAddresses[0])"
          foreach($boundary in $Listboundaries)
             {
              
              $BoundResult= IsIpAddressInRange $ResourceID.IPAddresses[0] $boundary.Value.Split('-')[0] $boundary.Value.Split('-')[1]
               
              # If there is a match, we get out of the loop     
              if($BoundResult -eq $true)
                 {
                    Write-CMTracelog "$($ResourceID.IPAddresses[0]) in $($boundary.DisplayName) are: $BoundResult" -ForegroundColor Green                 
                  Break    
                 }
              else
                 {
                 $FalseCount ++
                 }
             }
         }
      # Devices that do not have IP go to a csv list of their own.
      else
         {
          Write-CMTracelog " $($ResourceID.name) not have ip assigned" -ForegroundColor Red
          $MyUser = New-Object System.Object
          $MyUser | Add-Member -type NoteProperty -name 'Device Name' -value $ResourceID.name
          $MyUser | Add-Member -type NoteProperty -name 'Device IP' -value 'without assigned IP'         
          $MyUser | Add-Member -type NoteProperty -name 'Boundary assigned' -value 'Without assigned boundary'       
          $ListadoClientesRepairs += $MyUser
         }
      # We compare the boundaries checked with failure vs. the number of boundaries, if they are equal the IP does not fall into any boundary and is reported.
      If($FalseCount -eq $Listboundaries.Count)
         {
          Write-Host "$($ResourceID.IPAddresses[0]) Not have assigned boundary" -ForegroundColor Red
          $MyUser = New-Object System.Object
          $MyUser | Add-Member -type NoteProperty -name 'Device name' -value $ResourceID.name
          $MyUser | Add-Member -type NoteProperty -name 'Device IP' -value $ResourceID.IPAddresses[0]      
          $ListadoFinal += $MyUser
         }
              
     }
 
 Write-CMTracelog 'Generating files with results'
 $TestPath= Test-Path -Path $PathFilesExport
 if($TestPath -eq $false){mkdir $PathFilesExport}
 $Date= Get-Date -Format "dd_MM_yyyy"
 $Append1 = $PathFilesExport+$TargetCollection+$Date+".csv"
 $Append2 = $PathFilesExport+"deviceswithoutip_"+$TargetCollection+"_"+$Date+".csv"
 $ListadoFinal | Export-Csv -Path  $Append1  -Force -NoTypeInformation
 $ListadoClientesRepairs | Export-Csv -Path $Append2 -Force -NoTypeInformation
 
 Write-CMTracelog 'Sending reports mail'
 $Subject = "Automated Task"
 $Title = "Device without associated boundary"
 $Message = "This task identifies hosts that have not been assigned a boundary in SCCM collection $TargetCollection, It also identifies devices that have not reported IP."

 
 [string[]]$attachment =  $Append1 , $Append2 
 send-MailHtml -SmtpServer $SmtpServer -Subject $Subject -Title $Title -Message $Message -ListAdress $ListAdress -attachment $attachment 