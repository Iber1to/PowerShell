...
Write-SimpleLog -Message "Searching for [$($installTitle)] MSI in [$($dirFiles)]."

If($MSIPath.Exists)
    {
    Write-SimpleLog -Message "[$($installTitle)] MSI found: [$($MSIPath.FullName)]."
    try {
        Write-SimpleLog -Message "Found [$($MSIPath.FullName)], now attempting to install [$($installTitle)]. With arguments: [$($Argumenlist)]"
        ...
        Start-Process -FilePath $MSIPath.FullName -ArgumentList $Argumenlist -Wait
