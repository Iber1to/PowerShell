...
try {
    Write-SimpleLog -Message "Attempting to uninstall [$($installTitle)]. Waiting for the process to complete..."
    Write-SimpleLog -Message "Uninstall file path: [$($UninstallFilePath)] with arguments: [$($Argumenlist)]"
    start-process -FilePath $UninstallFilePath -ArgumentList $Argumenlist -Wait -NoNewWindow
    Write-SimpleLog -Message "[$($installTitle)] uninstalled successfully."
    exit 0
}
catch {
    Write-SimpleLog -Message "Error uninstalling [$($installTitle)] : $($_.Exception.Message)" -LogLevel "ERROR"
    exit 1
}
#endRegion