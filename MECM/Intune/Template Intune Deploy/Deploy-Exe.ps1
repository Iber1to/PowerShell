...
Write-SimpleLog -Message "Starting installation of [$($installTitle)]."
Write-SimpleLog -Message "Searching for [$($installTitle)] executable in [$($dirFiles)]."
$ExePath = Get-ChildItem -Path "$dirFiles" -Include $ExeFile -File -Recurse -ErrorAction SilentlyContinue
If($ExePath.Exists)
    {
    Write-SimpleLog -Message "[$($installTitle)] executable found: [$($ExePath.FullName)]."
    try {
        Write-SimpleLog -Message "Found [$($ExePath.FullName)], now attempting to install [$($installTitle)]. With arguments: [$($Argumenlist)]"
        Write-SimpleLog "Installing [$($installTitle)]. This may take some time. Please wait..."
        Start-Process -FilePath $ExePath -ArgumentList $Argumenlist -Wait
        Write-SimpleLog -Message "[$($installTitle)] installed successfully."
        }
    catch {
    ...
    Write-SimpleLog -Message "[$($installTitle)] executable not found in [$($dirFiles)]." -LogLevel "ERROR"
    exit 1
}
#endRegion